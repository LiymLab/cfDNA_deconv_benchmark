...
def run_methylation_deconvolution(input_dir, ref_file, dis):
    ...
    cmd_args = [
        "python",
        "~/cfNOMe/methylation_deconvolution.py",
        ref_file,
        *bed_files,
        "--ineq",
        "--verbose",
...
def main():
    ...
    parser.add_argument('--input-dir', type=str, required=True, help='The directory containing input mixture files.')
    parser.add_argument('--ref-file', type=str, required=True, help='The path to the reference file.')
    parser.add_argument('--dis', type=str, required=True, choices=['uniform', 'crd'], help='The distribution type (either uniform or crd).')
