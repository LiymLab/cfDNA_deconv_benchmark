...
for (i in 1:length(plots)) {
  plots[[i]] <- plots[[i]] + theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm")) 
}
...
library(parallel) 
library(ggplot2)
Rdata = "/real_data/disease/disease_method_data.RData"#Tissue proportions of different datasets calculated using different methods
prefix = "disease"
out_dir ="/real_data/disease/"
...
legend("bottomright", legend = c(paste("CelFEER(AUC =", round(auc_celfeer, 2), ")"), 
