...
data_drop <- data_full[,-grep(tissue_meth,colnames(data_full),fixed = TRUE)]     #去除选择的cell type的meth信息对应的索引的数据
data_drop <- data_drop[,-grep(tissue_depth,colnames(data_drop),fixed = TRUE)]     #去除选择的cell type的depth信息对应的索引的数据
colnames(data_drop)[204:206] <- c('CHROM','METH','DEPTH')
data_name <- paste0('~/celfie_work2/celfie_drop_tissue_new/data_median_15_drop_',tissue,'/data_input.txt')
