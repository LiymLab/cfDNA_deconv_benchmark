...
tissue_drop <- c('Endothelium','Blood-Granul','Adipocytes','Blood-NK','Head-Neck-Ep')
...
for(i in tissue_drop){
  start <- 506 + 5 * grep(i,colnames_tissue) - 4     #记录选择的tissue的数据的起始索引位置
  end <- start + 4     #记录选择的tissue的数据的终止索引位置
  data_drop <- data_full[,-(start:end)]     #删除对应tissue的所有数据
  data_name <- paste0('~/celfeer_work2/celfeer_drop_tissue/data_median_15_drop_',i,'/data_input.txt')
  colnames_drop <- t(as.data.frame(colnames_all[-grep(i,colnames_all,fixed = TRUE)]))     #去除对应的tissue的colname并形成一个新的colnames
  col_name <- paste0('~/celfeer_work2/celfeer_drop_tissue/data_median_15_drop_',i,'/colnames.txt')
...
for(i in 1:length(tissues_drop)){
  ...
  colnames_drop <- t(as.data.frame(colnames_drop[-grep(tissue2,colnames_all,fixed = TRUE)]))     #依次删除一组tissue中的所有列名信息
  col_name <- paste0('~/celfeer_work2/celfeer_drop_tissues/data_median_15_drop_',tissue1,'_',tissue2,'/colnames.txt')
  data_name <- paste0('~/celfeer_work2/celfeer_drop_tissues/data_median_15_drop_',tissue1,'_',tissue2,'/data_input.txt')
